...
# Initialize MongoDB with certifi for SSL/TLS fix
@st.cache_resource
def init_db():
    client = pymongo.MongoClient(
        MONGO_URI,
        tlsCAFile=certifi.where()  # ✅ Fixes MongoDB Atlas SSL handshake error
    )
    db = client[DB_NAME]
    return db
...
if not st.session_state.authenticated:
    ...
    SERVERS = [s for s in ALL_SERVERS if s["url"] in USER_PERMISSIONS]
        ...
        if isinstance(result, dict):
            ...
            if any(key in result for key in ["pods", "services", "deployments", "configmaps", "secrets", "namespaces", "nodes"]):
                ...
                for resource_type, resources in result.items():
                    ...
                    elif isinstance(resources, dict) and "items" in resources:
