...
# ---------------- HELPERS ----------------
def check_jenkins_status(MCP_SERVER_URL: str):
    """Call MCP server to check Jenkins status"""
    try:
        payload = {
            "jsonrpc": "2.0",
            "method": "tools/call",
            "params": {"name": "jenkins_status", "arguments": {}},
            "id": 1,
        }
        resp = requests.post(f"{MCP_SERVER_URL}/mcp", json=payload, timeout=5)
        data = resp.json()
        result = data.get("result", {})
        return f"Jenkins Status: {result.get('status', 'UNKNOWN')}"
    except Exception as e:
...
def handle_user_prompt(user_prompt: str, server_url: str):
    """Decide whether to call tools or forward to Gemini."""
    # Very simple intent detection for Jenkins status
    if re.search(r"jenkins.*(up|down|status|running)", user_prompt.lower()):
        return check_jenkins_status(server_url)

    # Otherwise fallback to Gemini tool decision (your existing flow)
    if GEMINI_AVAILABLE:
        decision = ask_gemini_for_tool_decision(user_prompt, server_url)
        return f"💡 {decision.get('explanation', 'I will try to handle this...')}"
    else:
        return "⚠️ Gemini not available, cannot analyze prompt."
...
def understand_query_intent(query: str) -> Dict:
    ...
    try:
        ...
        elif "pipeline" in query_lower and "stage" in query_lower:
            job_name, build_number = extract_build_number(query)
            return {
                "operation": "get_build_stages",
...
def ask_jenkins_question(user_text: str):
    ...
    # Build logs
    ...
    elif operation == "get_build_stages":
        job_name = parameters.get("job_name", "")
        build_number = parameters.get("build_number", "last")
        ...
        result = call_mcp_tool("get_build_stages", {"job_name": job_name, "build_number": build_number})

        if "error" in result:
            return f"Error getting pipeline stages: {result['error']}"

        stages = result.get("result", {}).get("stages", [])
        if not stages:
            return f"No pipeline stages found for job '{job_name}' build #{build_number}."

        response = f"Pipeline stages for {job_name} #{build_number}:\n"
        for s in stages:
            response += f"- {s['name']} [{s['status']}]\n"
        return response
