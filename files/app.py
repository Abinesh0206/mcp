...
<style>
  ...
  .css-1v0mbdj, .css-1dp5vir {{ color: #111; }}
  .chat-bubble-user {{
    border-left: 4px solid {PRIMARY}; padding: 12px; margin: 8px 0; border-radius: 12px; background: #f5f9ff;
  }}
  .chat-bubble-bot {{
    border-left: 4px solid {ACCENT}; padding: 12px; margin: 8px 0; border-radius: 12px; background: #fff8f0;
  }}
  .history-item {{ cursor:pointer; padding:8px; margin:4px 0; border-radius:10px; border:1px solid #eee; }}
...
if "sessions" not in st.session_state:
    st.session_state.sessions = []      # list of {title, messages}
...
if user_text:
    ...
    # Route to MCP if matched, else to Ollama
