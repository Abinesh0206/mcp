...
def ask_gemini_for_tool_and_server(query: str, retries: int = 2) -> Dict[str, Any]:
    ...
    if not GEMINI_AVAILABLE:
        return {"tool": None, "args": None, "server": None, "explanation": "⚠️ No Gemini, try simple query."}
...
def main():
    ...
    if not decision.get("tool"):
        answer = "⚠️ Could not determine tool. Try again with cluster/argo/jenkins query."
    ...
    if not resp or "error" in resp:
        final_answer = f"⚠️ Execution failed: {resp.get('error', 'Unknown error')}"
    else:
